...
function Mode2(hour, Position, Width, Height, Bottom, Tsol, Rfloor, bodydirection, clothingAbsorptivity, epw) {
    // User-defined inputs (fixed single values):
    ...
    var lat = epw.latitude;
    var lon = epw.longitude;
    console.log(diffuseRad, directnormalRad, TimeZone, lat, lon);
    ...
    for (var j = 0.5; j < 4; j = j + 0.5) {
        for (var i = -wid_bound; i <= wid_bound; i = i + 0.5) {
            ...
            HourlyDeltaMRT = HNUSolar(Position, Width, Height, Bottom, Tsol, Rfloor, i, j, bodydirection, clothingAbsorptivity, diffuseRad, directnormalRad, SunAlt(lat, lon, TimeZone, hour), SunAzi(lat, lon, TimeZone, hour));
